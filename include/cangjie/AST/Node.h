...
enum class AnnotationKind {
    ...
    INTRINSIC,
    WHEN,
    /** Checked exceptions (experimental): '@AssumeThrows[E1, E2]' on an import. */
    ASSUME_THROWS,
    FASTNATIVE,
...
struct Annotation : public Node {
    ...
    /**
     * For '@AssumeThrows' on an import: the exception types imposed as a
     * 'throws' list on every call into the imported package. The bare form assumes 'Exception'.
     * Reuses the clause node shape. Optional, may be null.
     */
    OwnedPtr<ThrowsClause> assumeThrows;
    // For @Annotation.
    uint16_t target = 0;
...
/**
 * A ThrowsClause node represents a checked-exception capability clause (experimental, behind
 * `--experimental --enable-chexc`): `throws E`, `throws (E1, E2)`, `throws ()`; declarations
 * additionally allow the bare list form `throws E1, E2`.
 */

struct ThrowsClause : Node {
    Position throwsPos;                   /**< Position of the `throws` keyword. Generated by parser */
    Position leftParenPos;                /**< Position of '('. Optional, zero for the bare form. */
    std::vector<OwnedPtr<Type>> capTypes; /**< Capability (exception) types. Generated by parser */
    std::vector<Position> commaPosVector; /**< Positions of ','. Generated by parser */
    Position rightParenPos;               /**< Position of ')'. Optional, zero for the bare form. */
    /**
     * The `...` marker of a declaration clause: the listed entries are declared
     * and capability parameter inference stays enabled, its result unioned with them. Never set
     * for a functional type's clause, whose list is always complete.
     */
    bool hasEllipsis{false};
    ThrowsClause() : Node(ASTKind::THROWS_CLAUSE)
    {
    }
};
...
struct FuncType : Type {
    ...
    Position rightParenPos;                 /**< Position of ')'. */
    /**< Effect `performs` clause (experimental). Precedes 'throws'. Optional. */
    OwnedPtr<ThrowsClause> performsClause;
    OwnedPtr<ThrowsClause> throwsClause; /**< Checked-exception clause (experimental). Optional, may be null. */
...
struct InheritableDecl : public Decl {
     ...
     *): exception capabilities the class or struct captures at construction for use
...
struct FuncBody : Node {
    ...
    /**< Effect `performs` clause (experimental). Precedes 'throws'. Optional. */
    OwnedPtr<ThrowsClause> performsClause;
    OwnedPtr<ThrowsClause>
        throwsClause;          /**< Checked-exception `throws` clause (experimental). Generated by parser, Optional */
